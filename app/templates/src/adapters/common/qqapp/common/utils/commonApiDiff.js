const getStorageSync = (key) => qq.getStorageSync(key)
const setStorageSync = (params) => qq.setStorageSync(params.key, params.data)
const removeStorageSync = (key) => qq.removeStorageSync(key)

const request = params => qq.request(params)
...
const getClipboardData = options => qq.getClipboardData(options)
 ...
 * path	        string		       否	打开的页面路径，如果为空则打开首页。path 中 ? 后面的部分会成为 query，在小程序的 App.onLaunch、App.onShow 和 Page.onLoad 的回调函数或小游戏的 qq.onShow 回调函数、qq.getLaunchOptionsSync 中可以获取到 query 数据。对于小游戏，可以只传入 query 部分，来实现传参效果，如：传入 "?foo=bar"。
 * extraData	  object		       否	需要传递给目标小程序的数据，目标小程序可在 App.onLaunch，App.onShow 中获取到这份数据。如果跳转的是小游戏，可以在 qq.onShow、qq.getLaunchOptionsSync 中可以获取到这份数据数据。
 * envVersion	  string	release	 否	要打开的小程序版本。仅在当前小程序为开发版或体验版时此参数有效。如果当前小程序是正式版，则打开的小程序必定是正式版。
 * success	    function		     否	接口调用成功的回调函数
 * fail	        function		     否	接口调用失败的回调函数
 * complete	    function		     否	接口调用结束的回调函数（调用成功、失败都会执行）
 */
const navigateToMiniProgram = options => qq.navigateToMiniProgram(options)
